...
export default function AnalysisUpload() {
  ...
  // Frame rate is now auto-detected by backend for optimal performance
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Frame rate is auto-detected by backend for optimal performance
      // No user selection needed - backend uses 6 fps which is optimal for gait analysis
    }
  }
      ...
      <div className="upload-section">
        ...
        {status !== 'completed' && (
          <>
            ...
            {/* Processing info - frame rate is auto-optimized */}
            {file && status === 'idle' && (
              <div className="processing-info" style={{
                ...
                <p style={{ margin: 0, fontSize: '0.9rem', color: '#2e7d32' }}>
                  ⚡ <strong>Optimized Processing:</strong> Frame rate and settings are automatically configured for fast, accurate gait analysis.
                </p>
              </div>
            )}
          </>
